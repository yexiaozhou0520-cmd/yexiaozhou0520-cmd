...
**1. 期权定价模型实现（Black-Scholes + 蒙特卡洛模拟）**
- 基于 Python 实现金融工程中两种主流的欧式期权定价方法：Black-Scholes 解析解与蒙特卡洛数值模拟，并对定价结果进行误差对比分析
- 运用 NumPy 与 SciPy 完成模型计算，使用 Matplotlib 实现期权价格对波动率的敏感性可视化
- 构建了标准化 GitHub 项目仓库，包含完整说明文档、可复现代码与运行结果截图，便于后续拓展与复用

仓库地址：https://github.com/yexiaozhou0520-cmd/option-pricing-project
...
**3. 宏观经济预测模型实现（随机森林 + 多特征重要性分析）**
- 基于 Python 构建多因子宏观经济预测模型，以 CPI、GDP 增速等核心指标为预测目标，整合货币供应量、失业率、汇率等多维度经济变量，实现完整的数据清洗、时序特征工程、模型训练与结果评估流程
- 采用随机森林算法完成建模，通过特征重要性分析量化各宏观变量对目标指标的影响权重，并使用 Matplotlib 实现经济趋势图、预测对比图与特征重要性分析图的可视化输出
- 项目构建了标准化的 GitHub 仓库，包含完整的 README 文档、可复现代码与结果截图，体现对金融时间序列建模、模型可解释性与工程化落地的实践能力
